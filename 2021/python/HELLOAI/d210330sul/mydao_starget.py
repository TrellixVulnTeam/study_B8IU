...
class MyDaoStarget:
    ...
    def insertAll(self, survey_id, mobiles, user_id):
        sql = mybatis_mapper2sql.get_child_statement(self.mapper, "insertAll")
        MyLog().getLogger().debug(sql)
        cnt = 0
        for mobile in mobiles:
            self.cs.execute(sql, (survey_id, mobile.replace("-", ""), user_id, user_id))
            cnt += self.cs.rowcount
        self.conn.commit()
