...
from d210311.mylog import MyLog
...
class MyDaoStarget:
    ...
    def myselect(self, survey_id):
        ...
        list = []
        for record in rs:
            list.append({'survey_id':record[0],'st_mobile':record[1],'complete_yn':record[2],'in_date':record[3],'in_user_id':record[4],'up_date':record[5],'up_user_id':record[6],'in_user_id':record[7]})
        return list
